...
{
    int ret = update_sd_from_mem("/dev/block/platform/dw_mmc.0/mmcblk1", offset, imgBase, imgSize);
    if (ret < 0)
        ret = update_sd_from_mem("/dev/block/platform/dw_mmc.0/mmcblk0", offset, imgBase, imgSize);
    return ret;
}
...
void Register_librecovery_updater_s5p4418_general() {
