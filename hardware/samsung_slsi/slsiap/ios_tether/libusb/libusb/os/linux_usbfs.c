...
{
	int res;
	//printf("## \e[31m PJSMSG \e[0m [%s():%s:%d\t] auto_detach_kernel_driver:%d \n", __func__, strrchr(__FILE__, '/')+1, __LINE__, handle->auto_detach_kernel_driver);

	if (handle->auto_detach_kernel_driver)
		res = detach_kernel_driver_and_claim(handle, iface);
	else
		res = claim_interface(handle, iface);

	//printf("## \e[31m PJSMSG \e[0m [%s():%s:%d\t] res:%d \n", __func__, strrchr(__FILE__, '/')+1, __LINE__, res);
