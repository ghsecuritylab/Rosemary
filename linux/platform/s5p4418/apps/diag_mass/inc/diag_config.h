...
#define	ENV_DIAG_DEBUG_FONT			"NX_DIAG_DEBUG_FONT"
#define	ENV_DIAG_DEBUG_FONT_SIZE	"NX_DIAG_DEBUG_FONT_SIZE"
...
public:
	...
	char *GetDebugFont()
	{
		...
		{
			char *env_value;
			env_value = getenv(ENV_DIAG_DEBUG_FONT);
			if( NULL == env_value )
				printf("Cannot Found : %s environment parameter!!!\n", ENV_DIAG_DEBUG_FONT );
			else
				m_DebugFont = strdup( env_value );
		}
		return m_DebugFont;
	}
	int GetDebugFontSize()
	{
		if( m_DebugFontSize == -1 )
		{
			char *env_value;
			env_value = getenv(ENV_DIAG_DEBUG_FONT_SIZE);
			if( NULL == env_value )
				printf("Cannot Found : %s environment parameter!!!\n", ENV_DIAG_DEBUG_FONT_SIZE );
			else
				m_DebugFontSize = atoi( env_value );
		}
