...
#endif
    printf(" MIPI LCD pllms: %08x, bandctl: %08x, pllctl : %08x, phyctl : %08x\n",pllpms,bandctl,pllctl,phyctl);
	mdelay(20);
				...
				struct disp_multily_param *pmly, struct disp_mipi_param *pmipi)
{
    ...
    // Added by ddanggzi
    NX_GPIO_SetOutputValue(PAD_GET_GROUP(MIPI_LCD_RESET), PAD_GET_BITNO(MIPI_LCD_RESET), 1);
	mdelay(10);
	NX_GPIO_SetOutputValue(PAD_GET_GROUP(MIPI_LCD_RESET), PAD_GET_BITNO(MIPI_LCD_RESET), 0);
	mdelay(10);
	NX_GPIO_SetOutputValue(PAD_GET_GROUP(MIPI_LCD_RESET), PAD_GET_BITNO(MIPI_LCD_RESET), 1);
	mdelay(100);
	// end
