...
//extern void dw_mci_notify_change(struct platform_device *dev, int state);
extern int sdhci_force_presence_change(struct platform_device *pdev, int state);

#define	sdmmc_channel	dwmci_dev_ch1
...
int dhd_wlan_init_gpio(void)
{
	...
	*/
	...
	//gpio_wl_reg_on = -1;
    gpio_wl_reg_on = CFG_WIFI_REG_ON_NUM;
	//gpio_wl_host_wake = -1;
	gpio_wl_host_wake = CFG_WIFI_REG_ON_NUM+1;
