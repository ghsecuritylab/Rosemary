...
#include <mach/soc.h>   // for nxp_soc_gpio_set_io_func()
...
// Added by ddanggzi-- for wifi
void mmc_force_presence_change(struct platform_device *pdev, int state)
{
	dw_mci_notify_change(pdev, state);
}
EXPORT_SYMBOL_GPL(mmc_force_presence_change);

int sdhci_force_presence_change(struct platform_device *pdev, int state){
...
EXPORT_SYMBOL_GPL(sdhci_force_presence_change);
