...
static struct snd_soc_codec *rt5640 = NULL;
static int codec_bias_level = 0;

static int rt5640_jack_status_check(void);
extern int rt5640_jack_insert;
...
static const struct snd_soc_dapm_widget rt5640_dapm_widgets[] = {
	SND_SOC_DAPM_SPK("Ext Spk", rt5640_spk_event),
...
static int rt5640_jack_status_check(void)
{
	...
	if (!level) {
		...
		if( rt5640_jack_insert )
			rt5640_headset_detect(rt5640, 0);
		...
		rt5640_jack_insert = 1;
		//es8316_mono_en(0);

		rt5640_headset_detect(rt5640, 1);
...
static struct snd_soc_dai_link rt5640_dai_link = {
	...
	.codec_dai_name = "rt5640-aif1",		/* rt5640_dai's name */
	.codec_name 	= "rt5640.0-001c",		/* rt5640_i2c_driver name + '.' + bus + '-' + address(7bit) */
	.ops 			= &rt5640_ops,
	.symmetric_rates = 1,
...
static int rt5640_probe(struct platform_device *pdev)
{
	...
	pr_debug("rt5640-dai: register card %s -> %s\n",
...
static struct platform_driver rt5640_driver = {
	...
	.probe		= rt5640_probe,
	.remove		= __devexit_p(rt5640_remove),
};
module_platform_driver(rt5640_driver);

MODULE_AUTHOR("hsjung <hsjung@nexell.co.kr>");
MODULE_DESCRIPTION("Sound codec-rt5640 driver for the SLSI");
MODULE_LICENSE("GPL");
