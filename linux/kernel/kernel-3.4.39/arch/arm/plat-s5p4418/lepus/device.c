...
// Added by ddanggzi for audio
#elif defined(CONFIG_SND_CODEC_RT5640)

#include <linux/i2c.h>

#define	RT5640_I2C_BUS		(0)
...
NXE2000_PDATA_INIT(dc2,      0,	1000000, 2000000, 1, 1, 1050000, 1,  4);	/* 1.1V CORE */
...
// Modified by ddanggzi
            ...
            { PAD_GPIO_E +  7, NX_GPIO_PADFUNC_1 }, // ethernet_txd
          //  { PAD_GPIO_E +  7, NX_GPIO_PADFUNC_2},  // camera_vsysnc
...
static int back_camera_power_enable(bool on)
{
    ...
    // Added by ddanggzi
	//nxp_soc_gpio_set_io_func(io, NX_GPIO_PADFUNC_1); 
   // nxp_soc_gpio_set_io_dir(io, 1);
	...
	nxp_soc_gpio_set_io_func(reset_io, NX_GPIO_PADFUNC_0);
...
static int front_camera_power_enable(bool on)
{
    ...
    nxp_soc_gpio_set_io_dir(io, 1);
	nxp_soc_gpio_set_io_func(io, NX_GPIO_PADFUNC_0);
...
static struct dw_mci_board _dwmci1_data = {
	...
	.cd_type 		= DW_MCI_CD_NONE,
	.pm_caps        = MMC_PM_KEEP_POWER | MMC_PM_IGNORE_PM_NOTIFY,
	.clk_dly        = DW_MMC_DRIVE_DELAY(0xbf) | DW_MMC_SAMPLE_DELAY(0x7f) | DW_MMC_DRIVE_PHASE(1) | DW_MMC_SAMPLE_PHASE(0),
#if defined (CONFIG_MMC_DW_IDMAC) && defined (CONFIG_MMC_NXP_CH1_USE_DMA)
	.mode			= DMA_MODE,
#else
    .mode 			= PIO_MODE,
 ...
 */
#if defined(CONFIG_RFKILL_NXP)
// Modified by ddanggzi -- for bluetooth
#if 0
...
#else
// rfkill_plat_data is used in nxp_rfkill_setup() of rfkill-nxp.c
struct rfkill_dev_data  rfkill_dev_data =
{
	.supply_name 	= "vgps_3.3V",	// vwifi_3.3V, vgps_3.3V
...
/*------------------------------------------------------------------------------
 * Bluetooth wake pin control.
 */
void bluetooth_wake_set(int enable)
{
	unsigned int io = CFG_GPIO_BT_UART_RST;

	nxp_soc_gpio_set_io_dir(io, 1);
	
	nxp_soc_gpio_set_out_value(CFG_GPIO_BT_WAKE, 0);
	nxp_soc_gpio_set_out_value(CFG_GPIO_BT_WAKE, enable);

	nxp_soc_gpio_set_io_func(io, NX_GPIO_PADFUNC_1);
	nxp_soc_gpio_set_out_value(io, 1);		          //RTS = high
	nxp_soc_gpio_set_out_value(CFG_GPIO_BT_RESET, 0);  // Reset = low
	
	msleep(600);
	nxp_soc_gpio_set_io_dir(io, 1);
    nxp_soc_gpio_set_io_func(io, NX_GPIO_PADFUNC_1);
	nxp_soc_gpio_set_out_value(io, 0);                // RTS = low(Active Low)
	
	nxp_soc_gpio_set_out_value(CFG_GPIO_BT_RESET, 1);     // Reset = high
	nxp_soc_gpio_set_io_func(io, NX_GPIO_PADFUNC_2);  //GPIO --> RTS

}
EXPORT_SYMBOL(bluetooth_wake_set);
...
// Added by ddanggzi -- for audio
#elif defined(CONFIG_SND_CODEC_RT5640) || defined(CONFIG_SND_CODEC_RT5640_MODULE)
...
// ddanggzi -- end
    ...
    printk("plat: add touch(gt9147) device\n");
...
#if defined(CONFIG_TOUCHSCREEN_GOODIX)
	printk("plat: add touch(gt9147) device\n");
	i2c_register_board_info(GT9147_I2C_BUS, &goodix_i2c_bdi, 1);
#endif
